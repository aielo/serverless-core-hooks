...
class CoreHooks {
  ...
  load() {
    ...
    sls.run = function () {
      self.activate(); // core objects active (i.e. ready for hooks)
    ...
    const loadAP = pm.loadAllPlugins;                // preserved
    pm.loadAllPlugins = function () {
      pm.plugins = [];
      pm.commands = {};
      pm.aliases = {};
      pm.hooks = {};
      CHObjects.extract(pm).loadAllPlugins = loadAP; // restored
    ...
    _.each(objects, (name) => {
      if (name === "serverless") {  // serverless itself
        sls = CHObjects.create(name, sls);
        self.coreObjects[name] = sls;
      } else {                      // serverless child
        sls[name] = CHObjects.create(name, sls[name]);
    ...
    CHHooks.pluginManager = CHObjects.extract(sls.pluginManager);
