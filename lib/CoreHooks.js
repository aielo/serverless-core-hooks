...
class CoreHooks {
  ...
  // Turns on interception of all proxies
  proxyOn() {
    this.proxyActive = true;
  }

  // Turns off interception of all proxies
  proxyOff() {
  ...
  findHooks(trigger) {
    ...
    this.log("hooks " + trigger + " " + JSON.stringify(hooks));
    return hooks;
  }
  ...
  executeHooks(trigger, ...args) {
    ...
    for (const hook of hooks) {
      let info = hook.pluginName + ":" + hook.hook;
      this.log("hook exec " + trigger + " [" + info + "]");
      hook.hook(...args);
    }
  }
