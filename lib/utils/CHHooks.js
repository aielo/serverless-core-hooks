...
const CHHooks = {
  ...
  attach(target, key, action, value) {
    ...
    CHHooks.log(message);
    this.execute("before:" + event);
  },

  // Executes hooks for provided trigger (e.g. after:serverless:version)
  execute(trigger) {
    _.each(this.find(trigger), (hook) => {
      CHHooks.log("hooks exec " + trigger + " " + util.inspect(hook));
      hook.hook();
    });
  },
