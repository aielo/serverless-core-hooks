...
const CHObjectHandler = {
  get(target, key) {
    if (this.isCHEvent(target, key)) {
      // TODO: get property post-hooks
      // target[key] = CHObjects.CHHooks.attach(target, key, "get");
      CHObjects.log("object event " + target._chName + ":" + key);
      CHObjects.CHHooks.attach(target, key, "get");
    }
    return target[key];
  },
  ...
  set(target, key, value) {
    if (this.isCHEvent(target, key)) {
      ...
      // TODO: get value post-hooks
      // value = CHObjects.CHHooks.attach(target, key, "set", value);
      CHObjects.CHHooks.attach(target, key, "set", value);
    }
    target[key] = value;
    return true;
  }
